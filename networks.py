...
class UBlock(BasicBlock):
    expansion = 1

    def __init__(self, in_planes, planes, stride=1, norm='instancenorm', pertubration=None, uncertainty=0.0):
        super(UBlock, self).__init__(in_planes, planes, stride, norm)
    ...
    def forward(self, x):
        out = self.conv1(x)
        out = F.relu(self.bn1(self.pertubration(out)))
        out = self.conv2(out)
        out = self.bn2(self.pertubration(out))
        out += self.shortcut(x)
        out = F.relu(out)
        return out
...
class UResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, num_blocks, stride, pertubration, uncertainty):
        ...
        layers = []
        for stride in strides:
            layers.append(block(self.in_planes, planes, stride, self.norm, pertubration, uncertainty))
            self.in_planes = planes * block.expansion
        return nn.Sequential(*layers)
